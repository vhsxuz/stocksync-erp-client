...
const AddItem = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (!vRes.ok || !cRes.ok) throw new Error('Failed to fetch reference data');

      const [vData, cData] = (await Promise.all([vRes.json(), cRes.json()])) as [Vendor[], Category[]];
      setVendors(vData);
      setCategories(cData);
    };
    ...
    fetchAll().catch((e) => {
      console.error(e);
      alert('Failed to load vendors/categories');
    });
  }, []);
